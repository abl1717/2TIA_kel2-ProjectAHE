import { FaBook, FaWallet, FaSignOutAlt } from "react-icons/fa";
...
export default function Sidebar({ setSidebarOpen }) {
  const menus = [
    ...
    { name: "Pengajar", path: "/owner/pengajar", icon: <BsPeopleFill /> },
    { name: "Modul", path: "/owner/modul", icon: <FaBook /> },
    { name: "Keuangan", path: "/owner/keuangan", icon: <FaWallet /> },
  ];

  const menuClass = ({ isActive }) =>
    `flex h-13 items-center rounded-2xl px-3 transition-colors duration-100
    ${
      isActive
        ? "bg-white/65 text-[#180161] font-bold shadow-md backdrop-blur-xl"
  ...
  return (
    <aside
      ...
      className="group fixed left-0 top-0 z-50 flex h-screen w-20 flex-col overflow-hidden rounded-r-[32px] border-r border-white/25 bg-gradient-to-b from-[#180161]/90 via-[#4F1787]/80 to-[#EB3678]/75 px-3 py-5 shadow-[0_30px_90px_rgba(24,1,97,0.35)] backdrop-blur-2xl transition-[width] duration-150 ease-out hover:w-64"
    >
      <div className="pointer-events-none absolute -left-20 top-8 h-48 w-48 rounded-full bg-white/15 blur-3xl"></div>
      <div className="pointer-events-none absolute bottom-16 right-0 h-44 w-44 rounded-full bg-[#FB773C]/25 blur-3xl"></div>
